...
- Keep cycles small enough that one cycle can be completed and verified in one
  gralph iteration.
- Agent names below assume a repo agent table with `frontend-engineer` and
  `technical-writer`. Replace them before running gralph if your target repo
  uses different agent names.
