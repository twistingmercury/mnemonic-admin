...
the current environment. If additional targeted checks are needed to establish
confidence, run the smallest relevant ones.

Verification order:

1. targeted checks for the touched code
2. the cycle’s declared `Verify`
3. broader checks only if required by the change

If verification fails:

- fix the problem if it is within the cycle scope
- rerun verification
- do not mark the cycle complete until it passes
